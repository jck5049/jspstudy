...
/*
 	...
 	3. HttpSevlet 클래스를 상속 받는다.
 	4. Jsp/Sevlet Container인 Tomcat에 의해서 실행된다. (Tomcat이 없으면 컴파일 오류 발생)
...
public class HelloSevlet extends HttpServlet {
	...
	/*
	 	4. doGet()
	 		...
	 		2) GET 방식의 요청의 예시
	 			(1) <a href="http://localhost:9090/01_Servlet/HelloServlet">				태그 방식
	 			(2) <form action="http://localhost:9090/01_Servlet/HelloServlet">			태그 방식
	 			(3) location.href='http://localhost:9090/01_Servlet/HelloServlet'			자바스크립트 방식
	 			(4) open('http://localhost:9090/01_Servlet/HelloServlet', '타겟', '특징')	자바스크립트 방식
	 			(5) $.ajax({
	 					type: 'GET',
